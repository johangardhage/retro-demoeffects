...
//
// This file's technique: real linear-light RGB, the same engine juggler3.cpp
// uses - ambient, diffuse and reflection added into one colour accumulator
// per ray, rather than an index picked off a fixed palette ramp, so a chrome
// ball genuinely reddens against the torso instead of that being
// approximated. The one exception is Eric Graham's real glint() highlight
// (see TraceScene and MINGLINT), which is not additive at all: found, it
// replaces ambient and diffuse outright with flat white; not found, it
// contributes nothing, ever.
//
// One camera ray a pixel, retraced live from continuous time every
// displayed frame, with no antialiasing at all - juggler3.cpp's four
// samples and its quantize-then-vote combine step are the only difference
// between the two files, so a silhouette edge here is a hard cut rather
// than a blend. That single sample a pixel is what keeps this faster than
// juggler3.cpp, though slower than picking a ramp index directly the way
// this file used to.
//
...
void DEMO_Render(double time, double deltatime)
{
	...
	for (int y = 0; y < RETRO_HEIGHT; y++) {
		float b = ratio * (RETRO_HEIGHT / 2.0f - (y + 0.5f));
		for (int x = 0; x < RETRO_WIDTH; x++) {
			float a = ratio * (x + 0.5f - RETRO_WIDTH / 2.0f);
